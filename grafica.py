...
with col_filtros:
            ...
            if "eje_x" not in st.session_state:
                st.session_state["eje_x"] = "%USGAAP 90 PONDERADO"
            
            if "ejes_y" not in st.session_state:
                st.session_state["ejes_y"] = ["RRR"]
            
            # Widgets para los filtros del gráfico de dispersión
            st.sidebar.header("Filtros para el Gráfico de Dispersión")
            
            st.session_state["hojas_seleccionadas_disp"] = st.sidebar.multiselect(
                "Selecciona las hojas:",
                list(dfs.keys()),
                default=st.session_state["hojas_seleccionadas_disp"]
            )
            
            st.session_state["negocios_disp"] = st.sidebar.multiselect(
                "Selecciona los negocios:",
                options=list(colores_negocios_variantes.keys()),
                default=st.session_state["negocios_disp"]
            )
            
            st.session_state["plazo_meses_disp"] = st.sidebar.selectbox(
                "Selecciona el plazo:",
                options=["Todos"] + list(range(1, 60)),
                index=(["Todos"] + list(range(1, 60))).index(st.session_state["plazo_meses_disp"])
            )
            
            if st.session_state["hojas_seleccionadas_disp"] and st.session_state["negocios_disp"]:
                df_seleccionado = pd.concat(
                    [dfs[hoja] for hoja in st.session_state["hojas_seleccionadas_disp"]],
                    ignore_index=True
                )
                df_filtrado_por_negocio = df_seleccionado[df_seleccionado["NEGOCIO"].isin(st.session_state["negocios_disp"])]
                ...
                st.session_state["departamentos_seleccionados"] = st.sidebar.multiselect(
            ...
            if "Todos" in st.session_state["departamentos_seleccionados"]:
                departamento_disp = "Todos"
            else:
                departamento_disp = st.session_state["departamentos_seleccionados"]
            
            opciones_columnas = ["%USGAAP 90 PONDERADO", "RRR", "RRR (con margen)", "MARGEN", "TASA ACTIVA PONDERADA"]
            
            st.session_state["eje_x"] = st.sidebar.selectbox(
                "Selecciona la variable para el Eje X:",
                opciones_columnas,
                index=opciones_columnas.index(st.session_state["eje_x"])
            )
            
            st.session_state["ejes_y"] = st.sidebar.multiselect(
                "Selecciona las variables para el Eje Y:",
                opciones_columnas,
                default=st.session_state["ejes_y"]
            )
...
with col_graficos:
            if ejes_y:
                crear_grafico_dispersión_multiple(
                    ...
                    departamento_disp,
                    st.session_state["plazo_meses_disp"],
